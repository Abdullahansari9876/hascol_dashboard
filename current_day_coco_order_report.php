<?php
// Hascol OMC Operations Command Center - Local Current Day COCO Order Report
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Hascol OMC - Current Day COCO Order Report</title>
...
<body class="flex h-screen overflow-hidden text-xs">
        ...
        <div class="flex-1 overflow-y-auto p-4" id="pageContent">
            ...
            <div class="panel-card p-4 mb-4">
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <div>
                        <button onclick="fetchReport()" class="btn-primary" style="padding:0 24px;">
            ...
            <div class="panel-card overflow-hidden">
                <div class="p-3 border-b" style="border-color: var(--border-color);">
                    <div class="report-header">
                        Orders Current Day of <span id="report_date"><?php echo date('Y-m-d'); ?></span>
                    </div>
                </div>
                    ...
                    <table id="reportTable" class="display" style="width:100%;">
                        <thead>
                            <tr>
                                ...
                                <th>Site Name</th>
                                <th>Depot</th>
                                <th>PMG <span class="unit-label">(KL)</span></th>
                                <th>HSD <span class="unit-label">(KL)</span></th>
                                <th>HASRON <span class="unit-label">(KL)</span></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
    ...
    <script>
        $(document).ready(function() {
            var datePicker = flatpickr("#fromdate", {
                dateFormat: "Y-m-d",
                maxDate: new Date(),
            ...
            $('#customSearchInput').on('keyup', function() {
                if ($.fn.DataTable.isDataTable('#reportTable')) {
                    $('#reportTable').DataTable().search($(this).val()).draw();
                }
            });
                ...
                success: function(response) {
                    if (response && Array.isArray(response) && response.length > 0) {
                        ...
                        $.each(response, function(index, data) {
                            var pmg = data.PMG ? (data.PMG / 1000).toFixed(2) : '0.00';
                            var hsd = data.HSD ? (data.HSD / 1000).toFixed(2) : '0.00';
                            var hasron = data.HASRON ? (data.HASRON / 1000).toFixed(2) : '0.00';
                        ...
                        showToast('No data found for this date.', 'error');
                        initializeDataTable([]);
                    }
                },
                error: function(xhr, status, error) {
                    console.error('API Error:', status, error);
                    showToast('Failed to load report. Please refresh.', 'error');
                    initializeDataTable([]);
                }
            });
        }
        ...
        function initializeDataTable(data) {
            ...
            var reportDate = $('#report_date').text();
            ...
            reportDataTable = $('#reportTable').DataTable({
                ...
                buttons: [
                    ...
                    { extend: 'excelHtml5', text: '<i class="fa-regular fa-file-excel"></i> Excel', className: 'toolbar-btn', exportOptions: { columns: [0,1,2,3,4,5,6] }, title: 'Orders_Current_Day_' + reportDate },
                    { extend: 'csvHtml5', text: '<i class="fa-regular fa-file-csv"></i> CSV', className: 'toolbar-btn', exportOptions: { columns: [0,1,2,3,4,5,6] }, title: 'Orders_Current_Day_' + reportDate },
                    { extend: 'pdfHtml5', text: '<i class="fa-regular fa-file-pdf"></i> PDF', className: 'toolbar-btn', exportOptions: { columns: [0,1,2,3,4,5,6] }, title: 'Orders Current Day - ' + reportDate, orientation: 'landscape', pageSize: 'A4' },
                    { extend: 'print', text: '<i class="fa-solid fa-print"></i> Print', className: 'toolbar-btn', exportOptions: { columns: [0,1,2,3,4,5,6] }, title: 'Orders Current Day - ' + reportDate }
                ],
                pageLength: 10,
                ...
                order: [[0, 'asc']],
                language: {
                    emptyTable: '<div class="text-center py-8 text-gray-500"><i class="fa-solid fa-chart-bar text-2xl block mb-2"></i>No data found for this date</div>',
        ...
        function populateColumnDropdown() {
            ...
            columnConfig.forEach(function(col) {
                let isVisible = true;
                try { isVisible = reportDataTable.column(col.idx).visible(); } catch(e) { isVisible = true; }
                const item = `
        ...
        function toggleColumnVisibility(colIdx) {
            ...
            try {
                const isVisible = reportDataTable.column(colIdx).visible();
                reportDataTable.column(colIdx).visible(!isVisible);
        ...
        function selectAllColumns() {
            ...
            try {
                columnConfig.forEach(function(col) {
                    if (!reportDataTable.column(col.idx).visible()) {
                        reportDataTable.column(col.idx).visible(true);
                    }
        ...
        function deselectAllColumns() {
            ...
            try {
                columnConfig.forEach(function(col) {
                    if (reportDataTable.column(col.idx).visible()) {
                        reportDataTable.column(col.idx).visible(false);
                    }
