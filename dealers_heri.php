<?php
// Hascol OMC - Retail Hierarchy Management (Standalone)
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    <meta charset="utf-8" />
    <title>Retail Hierarchy | Hascol OMC</title>
...
<body class="flex h-screen overflow-hidden text-sm">
            ...
            <div class="flex flex-wrap justify-between items-center mb-4 gap-3">
                <div>
                    <h2 class="text-heading font-semibold text-lg tracking-wide uppercase">
                        <i class="fa-solid fa-sitemap mr-2 text-blue-500"></i>Retail Hierarchy
                    </h2>
                    <p class="text-xs text-gray-500 mt-0.5">Manage retail hierarchy and assign GRM, RM, TM</p>
                </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel">
        <div class="offcanvas-header border-bottom">
            <h5 id="offcanvasRightLabel" class="text-heading font-semibold offcanvas-title">
                <i class="fa-solid fa-plus-circle mr-2 text-blue-500"></i>Create Retail Hierarchy
            </h5>
        ...
        <div class="offcanvas-body">
            ...
            <div class="form-group">
                <label class="form-label">Region <span class="text-muted-small">(Select all stations under specific
                        region)</span></label>
                <select class="form-select" id="regions" name="regions" onchange="getRegionDealers(this.value)">
            ...
            <form method="post" id="hierarchy_form" enctype="multipart/form-data">

                <div class="form-group">
                    <label class="form-label">Stations <span class="text-red-500">*</span></label>
                    <select class="multiple_select" id="dealers" name="dealers[]" multiple="multiple"
                        style="width:100%;">
                    </select>
                </div>

                <div class="form-group">
                    <label class="form-label">GRM <span class="text-red-500">*</span></label>
                    <select class="form-select" id="zm" name="zm" onchange="getZmTm(this.value)" required>
                        <option value="">Select GRM</option>
                    </select>
                </div>

                <div class="form-group">
                    <label class="form-label">RM <span class="text-red-500">*</span></label>
                    <select class="form-select" id="tm" name="tm" onchange="getTmAsm(this.value)" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" id="asm" name="asm">
                ...
                <div class="flex gap-3 mt-6 pt-4 border-t">
                    <button type="submit" class="btn-success flex-1 justify-center" id="products_btn">
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.53/vfs_font s.js"></script>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#hierarchy_form').on('submit', function (event) {
        ...
        function initializeDataTable() {
            ...
            dataTable = $('#myTable').DataTable({
                ...
                pageLength: 10,
                language: {
                    emptyTable: 'No retail hierarchy found',
        ...
        function fetchTable() {
            var url = API_BASE + 'get/get_dealers_with_users.php?key=' + API_KEY;
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    if (status === 'timeout') {
                        dataTable.row.add([
                            '⏱️ Server taking too long. Please refresh the page.',
                            '',
                            '',
                            '',
                            '',
                        ...
                        Swal.fire({
                            icon: 'warning',
                            title: 'Timeout',
                            text: 'Server is taking too long. Please refresh the page.',
                            timer: 3000,
                        ...
                        dataTable.row.add([
                            '❌ API Not Found. Please check API URL.',
                            '',
                            '',
                            '',
                            '',
        ...
        function loadAllSelect() {
            var urlZm = API_BASE + 'get/get_zm.php?key=' + API_KEY;
            ...
            $.ajax({
                ...
                error: function (error) {
                    console.error('Error fetching ZM:', error);
                }
            });

            var urlRegions = API_BASE + 'get/get_dealers_region.php?key=' + API_KEY;
        ...
        function loadDealers() {
            var url = API_BASE + 'get/dealers.php?key=' + API_KEY + '&pre=' + PRIVILEGE + '&user_id=' + USER_ID;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        $.each(data, function (index, item) {
                            var optionText = item.name + ' - ' + (item.sap_no || '');
                            select.append($('<option>', {
                                value: item.id,
                                text: optionText
                            }));
                        });
                        $('.multiple_select').trigger('change');
                    }
                },
                error: function (error) {
        ...
        function getZmTm(id) {
            ...
            var url = API_BASE + 'get/individual_tm_of_zm.php?key=' + API_KEY + '&zm_id=' + id;
            ...
            $.ajax({
                ...
                timeout: 30000,
                success: function (data) {
                    console.log('TM Data for ZM:', data);
                    var select = $('#tm');
                    select.empty();
                    select.append($('<option>', {
                        value: '',
        ...
        function getTmAsm(id) {
            ...
            var url = API_BASE + 'get/individual_asm_of_tm.php?key=' + API_KEY + '&tm_id=' + id;
            ...
            $.ajax({
                ...
                timeout: 30000,
                success: function (data) {
                    console.log('ASM Data for TM:', data);
                    var select = $('#asm');
                    select.empty();
                    select.append($('<option>', {
                        value: '',
        ...
        function getRegionDealers(region) {
            ...
            var url = API_BASE + 'get/dealers_region.php?key=' + API_KEY + '&region=' + region;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        $.each(data, function (index, item) {
                            dealerIds.push(parseInt(item.id));
                        });
                    }
                    $('#dealers').val(dealerIds).trigger('change');
                },
                error: function (error) {
        ...
        function saveHierarchy() {
            var formData = $('#hierarchy_form').serialize();
            console.log('Form Data:', formData);

            var submitBtn = $('#products_btn');
            submitBtn.prop('disabled', true);
            submitBtn.html('<i class="fa-solid fa-spinner fa-spin"></i> Saving...');

            var url = API_BASE + 'update/multiple_dealers_heri.php';
        ...
        function openAddOffcanvas() {
            ...
            $('#hierarchy_form')[0].reset();
            $('.multiple_select').val(null).trigger('change');
            $('#offcanvasRightLabel').html('<i class="fa-solid fa-plus-circle mr-2 text-blue-500"></i>Create Retail Hierarchy');
            $('#products_btn').html('<i class="fa-regular fa-floppy-disk"></i> Save');
