<?php
// Hascol OMC - Survey Questions Engineering - Standalone
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    <meta charset="utf-8" />
    <title>Survey Questions Engineering | Hascol OMC</title>
...
<body class="flex h-screen overflow-hidden text-sm">
            ...
            <div class="flex flex-wrap justify-between items-center mb-4 gap-3">
                <div>
                    <h2 class="text-heading font-semibold text-lg tracking-wide uppercase">
                        <i class="fa-solid fa-list-check mr-2 text-blue-500"></i>Survey Questions Engineering
                    </h2>
                    <p class="text-xs text-gray-500 mt-0.5">Manage survey questions for engineering</p>
                </div>
                    ...
                    <div class="question-row">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div class="form-group mb-0">
                                <label class="form-label">Questions <span class="text-red-500">*</span></label>
                                <input type="text" class="form-input" id="questions" name="questions[]" placeholder="Enter question" required>
                            </div>
                            <div class="form-group mb-0">
                                <label class="form-label">Image Required</label>
                                <select class="form-select" name="file_req[]" id="file_req">
                            ...
                            <div class="form-group mb-0">
                                <label class="form-label">Action Time (Hours)</label>
                                <input type="number" class="form-input" id="action_time" name="action_time[]" step="1">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
                    ...
                    $(wrapper).append(
                        '<div class="question-row">' +
                            '<div class="grid grid-cols-1 md:grid-cols-2 gap-3">' +
                                ...
                                '<div class="form-group mb-0">' +
                                    '<label class="form-label">Action Time (Hours)</label>' +
                                    '<input type="number" class="form-input" name="action_time[]" step="1">' +
                                '</div>' +
                            '</div>' +
            ...
            dataTable = $('#myTable').DataTable({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        className: 'dt-button',
                        title: 'Survey_Questions_Eng_Export'
                    },
                    {
                        extend: 'csvHtml5',
                    ...
                    },
                    {
                        ...
                        className: 'dt-button',
                        title: 'Survey Questions Engineering',
        ...
        function fetchTable() {
            var url = API_BASE + 'get/survey_questions_eng.php?key=' + API_KEY + '&id=1';
                ...
                .then(response => {
                    ...
                    if (Array.isArray(response) && response.length > 0) {
                        $.each(response, function (index, data) {
                            ...
                            var fileRequired = (data.file == 'required') ? 'Required' : ((data.file == 'not_required') ? 'Not Required' : (data.file || 'N/A'));

                           dataTable.row.add([
    index + 1,
    data.name || 'N/A',
    data.question || 'N/A',
    '<span class="text-xs">' + fileRequired + '</span>',
    ...
    '</div>'
        ...
        function saveData() {
            ...
            var url = API_BASE + 'create/survey_questions_eng.php';
    ...
    $.ajax({
        type: 'POST',
        url: API_BASE + 'update/survey_question_eng.php',
        data: {
